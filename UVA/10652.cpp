...
void creaPoligono() {
    ...
    if (angulo != 0) {
        angulo = rad * angulo;
        dirX = rotate(dirX, -angulo);
        dirY = rotate(dirY, -angulo);
    }
    pt tmp = init;
    tmp = tmp + dirX * w/2;
    tmp = tmp + dirY * l / 2;
...
void resuelveCaso() {
    ...
    vector<pt> puntos = convexHull(vP);
    double areaTotal = areaPolygon(puntos);
    cout << fixed << setprecision(1) << (areas / areaTotal)*100 << " %\n";    
}
