...
// Convierte el de grado a radian
#define rad PI/180
...
bool resuelveCaso() {
    ...
    for (int i = 0; i < N; i++) {
        cin >> x1 >> y1 >> x2 >> y2;
        if (ccw({ x1,y1 }, { x2,y2 }, fuente)) {
            poligono = cutPolygon({ x1,y1 }, { x2,y2 }, poligono);
        }
        else {
            poligono = cutPolygon({ x2,y2 }, { x1,y1 }, poligono);
        }
    }

    cout << fixed << setprecision(3) << "Case #" << ++nCase << ": " << areaPolygon(poligono) << '\n';
